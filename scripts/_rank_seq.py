...
for idx, (target_code, target_name, date) in enumerate(CASES):
    print(f'[{idx+1}/10] {target_name} {date}...', flush=True)
        ...
        for fname in os.listdir(cdir):
            ...
            try:
                ...
                if j_val > 14 or not white_above:
                    continue
                ns = new_total(r)
                os_ = r.get('total_score', 0)
                if ts_code not in stock_new or ns > stock_new[ts_code]:
                    stock_new[ts_code] = ns
                if ts_code not in stock_orig or os_ > stock_orig[ts_code]:
                    stock_orig[ts_code] = os_
            except:
                continue
    ...
    perfect100_new = sum(1 for _, s in ranked_new if s >= 100)
    perfect95_orig = sum(1 for _, s in ranked_orig if s >= 95)
    
    r = {
        'stock': target_name,
        'code': target_code,
        'date': date,
        'total_candidates': total_stocks,
        'new_score': new_score,
        'new_rank': new_rank,
        'new_top20': new_rank <= 20 if new_rank > 0 else False,
        'new_perfect100': perfect100_new,
        'orig_score': orig_score,
        'orig_rank': orig_rank,
        'orig_top20': orig_rank <= 20 if orig_rank > 0 else False,
        'orig_perfect95': perfect95_orig,
    }
    results.append(r)
    print(f'  new: score={new_score:.0f} rank=#{new_rank}/{total_stocks} top20={r["new_top20"]} perfect100={perfect100_new}', flush=True)
    print(f'  orig: score={orig_score:.0f} rank=#{orig_rank}/{total_stocks} top20={r["orig_top20"]} perfect95={perfect95_orig}', flush=True)

# 保存结果
with open('outputs/_rank_results.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)

print(f'\n{"="*100}')
print('FINAL SUMMARY')
print(f'{"="*100}')
print(f'{"Stock":<12}{"Date":<12}{"NewScore":>9}{"NewRank":>8}{"Total":>7}{"N-T20":>7}{"Perf100":>8}{"OrigScore":>10}{"OrigRank":>9}{"O-T20":>7}{"Perf95":>7}')
print('-' * 100)
for r in results:
    print(f'{r["stock"]:<12}{r["date"]:<12}{r["new_score"]:>9.0f}{r["new_rank"]:>8}{r["total_candidates"]:>7}{"Y" if r["new_top20"] else "N":>7}{r["new_perfect100"]:>8}{r["orig_score"]:>10.0f}{r["orig_rank"]:>9}{"Y" if r["orig_top20"] else "N":>7}{r["orig_perfect95"]:>7}')

new_t20 = sum(1 for r in results if r['new_top20'])
orig_t20 = sum(1 for r in results if r['orig_top20'])
print(f'\nNew scoring TOP20: {new_t20}/10')
print(f'Orig scoring TOP20: {orig_t20}/10')
